...
class YOLO_TF:
    ...
    def analyze(self, id_img, input_img, output_img, current_time):
        self.current_time = current_time
        self.id_img = id_img
        self.fromfile = input_img
        self.tofile_img = output_img
    ...
    def show_results(self, img, results):
        ...
        if len(results) == 0 and self.filewrite_db:
            self.cursor.execute("""
                INSERT INTO main_record (id, class_result, image_id, time)
                VALUES (NULL, 'none',""" + str(self.id_img) + "," + str(self.current_time) + """)
            """)
        for i in range(len(results)):
            ...
            if self.filewrite_db:
                self.cursor.execute("""
                    INSERT INTO main_record (id, class_result, x, y, w, h, confidence, image_id, time)
                    VALUES (NULL, '""" + results[i][0] + "'," + str(x) + "," + str(y) + "," + str(w) + "," + str(h) + "," + str(results[i][5]) + "," + str(self.id_img) + "," + str(self.current_time) + """)
                """)
